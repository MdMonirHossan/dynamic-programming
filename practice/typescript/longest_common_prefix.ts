...
class CommonPrefixLength{
    ...
    solution(){
        ...
        let prefix_hash = new Set();

        for(let n of this.arr1){
            while(n && !prefix_hash.has(n)){
                prefix_hash.add(n);
                n = Math.floor(n / 10);
            }
        }
...
const lengthObj = new CommonPrefixLength([1, 10, 10000, 565656252], [10000, 20000, 56565626]);
console.log(lengthObj.solution());